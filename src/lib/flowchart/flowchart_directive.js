...
//
    ...
    .directive('flowChart', function () {
        return {
            restrict: 'E',
            templateUrl: "lib/flowchart/flowchart_template.html",
            replace: true,
            scope: {
                chart: "=chart",
                scale: "=scale",
        ...
        return {
            ...
            link: function (scope, elem, attr) {
                ...
                var updateJson = function () {
                    if (scope.viewModel) {
                        var json = angular.toJson(scope.viewModel.data, 4);
                        $(elem).val(json);
                    }
                };
    ...
    // (which is possible, just not ideal).
    //
    .controller('FlowChartController', ['$scope', 'dragging', '$element', 'toastr', function FlowChartController($scope, dragging, $element, toastr) {
        ...
        $scope.mouseDown = function (evt) {
            ...
            dragging.startDrag(evt, {
                ...
                dragStarted: function (x, y) {
                    ...
                    $scope.dragSelectionStartPoint.x = startPoint.x / $scope.scale - $scope.translate.x / $scope.scale;
                    $scope.dragSelectionStartPoint.y = startPoint.y / $scope.scale - $scope.translate.y / $scope.scale;
                    if(evt.ctrlKey){
                        $scope.dragSelecting = true;
                        $scope.dragSelectionRect = {x: 0, y: 0, width: 0, height: 0};
                    }
                ...
                dragging: function (x, y) {
                        ...
                        curPoint.x = curPoint.x / $scope.scale - $scope.translate.x / $scope.scale;
                        curPoint.y = curPoint.y / $scope.scale - $scope.translate.y / $scope.scale;
                    if(evt.ctrlKey){
                        $scope.dragSelectionRect = {
                        ...
                        $scope.translate.x+=curPoint.x*$scope.scale-$scope.dragSelectionStartPoint.x*$scope.scale;
                        $scope.translate.y+=curPoint.y*$scope.scale-$scope.dragSelectionStartPoint.y*$scope.scale;
                    }
                },
                ...
                dragEnded: function () {
                    if($scope.dragSelecting){
                        $scope.dragSelecting = false;
                        $scope.chart.applySelectionRect($scope.dragSelectionRect, mdevt.ctrlKey);
        ...
        $scope.nodeMouseDown = function (evt, node) {
            ...
            dragging.startDrag(evt, {
                ...
                dragStarted: function (x, y) {
                    ...
                    lastMouseCoords = controller.translateCoordinates(x / $scope.scale - $scope.translate.x / $scope.scale, y / $scope.scale - $scope.translate.y / $scope.scale, evt);
                ...
                dragging: function (x, y) {
                    ...
                    var curCoords = controller.translateCoordinates(x / $scope.scale - $scope.translate.x / $scope.scale, y / $scope.scale - $scope.translate.y / $scope.scale, evt);
        ...
        $scope.connectorMouseDown = function (evt, node, connector, connectorIndex, isInputConnector) {
            ...
            dragging.startDrag(evt, {
                ...
                dragging: function (x, y, evt) {
                    ...
                    $scope.dragPoint1 = flowchart.computeConnectorPos(node, connectorIndex, isInputConnector);
                    $scope.dragPoint2 = {
                        x: startCoords.x / $scope.scale - $scope.translate.x / $scope.scale,
                        y: startCoords.y / $scope.scale - $scope.translate.y / $scope.scale
                    };
                    $scope.dragTangent1 = flowchart.computeConnectionSourceTangent($scope.dragPoint1, $scope.dragPoint2);
                    $scope.dragTangent2 = flowchart.computeConnectionDestTangent($scope.dragPoint1, $scope.dragPoint2);
                },
                ...
                dragEnded: function () {
                    ...
                    if ($scope.mouseOverConnector &&
                        ...
                        // Create a new connection or throw an error.
                        //
                        try {
                            $scope.chart.createNewConnection(connector, $scope.mouseOverConnector);
                        }
                        catch (err) {
                            toastr.error(err.message, "Cannot create connection :", {closeButton: true});
                        }
                    }
